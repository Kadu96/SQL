        ...
        WITH TAB_REF AS (
            SELECT 
                ...
                (CASE 
                    ...
                    SELECT CODPROD FROM TGFITE WHERE NUNOTA = P_NUNOTA AND SEQUENCIA = NITEM)) THEN 'S'
                    ELSE 'N'
                END) AS ATUAL_REF
            FROM  XMLTable
                (
            ...
            IF V_ATUAL = 'S' THEN
                IF T_REFPROD = 13 AND V_CODBARRA NOT LIKE '%SEM GTIN%' THEN
                    INSERT INTO TGFBAR (CODBARRA, CODPROD, DHALTER, CODUSU, CODVOL)
                        SELECT REFERENCIA, CODPROD, SYSDATE, 0, CODVOL FROM TGFPRO WHERE CODPROD = V_PRODUTO;
