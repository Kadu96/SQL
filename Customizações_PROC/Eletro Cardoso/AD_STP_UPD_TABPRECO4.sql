CREATE OR REPLACE PROCEDURE "AD_STP_UPD_TABPRECO4" (
...
BEGIN
    ...
    /*ALIMENTA A TABELA INTERMEDIARIA AD_TGFTABBASE COM OS DADOS BASICO DOS PRODUTOS DA TABELA DE PREÇO 4*/
    ...
    WITH TAB_P2 AS (
        ...
        WHERE 
            EXC.NUTAB = (SELECT MAX(NUTAB) FROM TGFTAB WHERE CODTAB = 4) AND
            PRO.ATIVO = 'S'
    )
    SELECT
        P2.SEQUENCIA,
        ...
        NVL(PR.VLRVENDA, P2.VLRVENDA) AS VLRVENDA,
    ...
    SET
        T.BASESUBST = (
            SELECT 
                (CASE WHEN ICM.ALIQSUBTRIB > 0 THEN (
                    (T.VLRVENDA + 
                        (CASE WHEN P.TEMIPIVENDA = 'S' THEN (T.VLRVENDA * I.PERCENTUAL / 100) ELSE 0 END)) * (1 + PEM.MVAPADRAO/100)) 
                    ELSE 0 END)
            FROM TGFPRO P
                LEFT JOIN TGFIPI I ON I.CODIPI = P.CODIPI
            ...
            SELECT 
                (CASE WHEN ICM.ALIQSUBTRIB > 0 THEN 
                    (((T.VLRVENDA + (CASE WHEN P.TEMIPIVENDA = 'S' THEN 
                        (T.VLRVENDA * I.PERCENTUAL / 100) ELSE 0 END)) * (1 + PEM.MVAPADRAO/100) * ICM.ALIQSUBTRIB/100) - 
        ...
        T.MVA = (SELECT PEM.MVAPADRAO FROM TGFPEM PEM WHERE PEM.CODPROD = T.CODPROD AND PEM.CODEMP = 2) 
    WHERE T.CODPROD > 0;

    UPDATE AD_TGFTABBASE T
    SET T.VLRBASE = (
        SELECT T.VLRBASE + (CASE 
            WHEN P.TEMIPIVENDA = 'S' THEN (T.VLRBASE * I.PERCENTUAL / 100) ELSE 0 END)
        FROM TGFPRO P
                LEFT JOIN TGFIPI I ON I.CODIPI = P.CODIPI
                LEFT JOIN TGFICM ICM ON ICM.IDALIQ = T.IDALIQICMS
        WHERE P.CODPROD = T.CODPROD
    ...
    /*INSERIR UMA NOVA LINHA NA TABELA DE PREÇO 4*/

    INSERT INTO TGFTAB (NUTAB,CODTAB,DTVIGOR,DTALTER,CODTABORIG,UTILIZADECCUSTO)
       VALUES ((SELECT MAX(NUTAB)+1 FROM TGFTAB),4,SYSDATE-1,SYSDATE,4,'N');

    INSERT INTO TGFEXC (NUTAB,CODPROD,VLRVENDA,TIPO,CODLOCAL)
       SELECT 
              (SELECT MAX(NUTAB) FROM TGFTAB WHERE CODTAB = 4),
              CODPROD,
              (VLRBASE),
              'V',
              0
    ...
    UPDATE TGFEXC E SET E.VLRVENDA = (
    SELECT VLRVENDA FROM TGFEXC WHERE NUTAB =(SELECT MAX(NUTAB) FROM TGFTAB WHERE CODTAB = 4) AND TO_CHAR(CODPROD) = (
        SELECT COMPLDESC FROM TGFPRO WHERE CODPROD = E.CODPROD))
            WHERE
                E.NUTAB = (SELECT MAX(NUTAB) FROM TGFTAB WHERE CODTAB = 4) AND
                E.CODPROD IN (SELECT CODPROD FROM TGFPRO WHERE MARCA = 'IPCN');
