CREATE OR REPLACE VIEW AD_TGFATR (NUFIN, CODPARC, CODTIPTIT, DTNEG, DTVENC, TOLERINADIMP) AS
SELECT
    F.NUFIN, F.CODPARC, F.CODTIPTIT, F.DTNEG, F.DTVENC, NVL(P.TOLERINADIMP, 2) 
FROM TGFFIN F
...
HAVING
    (CASE 
        ...
        WHEN F.CODBCO = 341 AND F.CODTIPTIT = 4 THEN
            (CASE WHEN TO_CHAR(F.DTVENC, 'D') = 6 THEN F.DTVENC + 3 ELSE F.DTVENC + 1 END)
        ELSE F.DTVENC + 1 END) + NVL(P.TOLERINADIMP, 2) < TRUNC(SYSDATE)
GROUP BY
    F.CODPARC, F.NUFIN, F.DTNEG, F.DTVENC, F.CODTIPTIT, F.CODBCO, P.TOLERINADIMP
ORDER BY
